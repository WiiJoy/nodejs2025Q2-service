...
export class ArtistService {
  ...
  update(id: string, dto: UpdateArtistDto): ArtistResponse {
    const updateArtist = this.database.artists.find(
      (artist) => artist.id === id,
    );
  ...
  remove(id: string): ArtistResponse {
    ...
    const inFavorites = this.database.favorites.artists.findIndex(
      (item) => item === id,
    );
    if (inFavorites !== -1) {
      this.database.favorites.artists.splice(inFavorites, 1);
    }
